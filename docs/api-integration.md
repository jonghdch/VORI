...
> 아직 화면에 붙지 않은 6개 도메인(가구·목표·영수증·예산·칭호·테마)을 React 에 연결하기 위한 문서.
...
### 1. 영수증 업로드는 아직 main 에서 1MB 를 넘기면 실패한다

multipart 상한 설정이 `jonghdch` 브랜치(`f5a78ac`)에만 있고 **main 에는 아직 없다.** 그 상태로는 휴대폰 사진(2~6MB)이 컨트롤러에 닿기 전에 413 으로 튕긴다.

그 커밋이 main 에 올라오면 해결된다. 그 전까지 영수증 화면은 붙여도 큰 사진으로는 테스트가 안 된다.
