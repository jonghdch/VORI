...
BASE_HOST, BASE_PORT = "127.0.0.1", 8000
OUT_PATH = Path(__file__).parent / "compare_ghij_result.json"
SEED_BASE = 1000
METHODS = ("G", "H", "I", "J")
_conn = None
...
def judge(m, exp, ctx):
    return _req("POST", "/judge", {
        "approach": m,
        "expense": {k: exp.get(k) for k in ("description", "amount", "category", "time_of_day", "date")},
        "user_context": ctx,
    ...
    for exp in expenses:
        for m in METHODS:
            try:
                ...
                sig[m]["error"] += 1
    return {
        "name": persona.name, "job": persona.job, "income": persona.monthly_income,
        "total": len(expenses),
...
def main():
    ...
    for i, p in enumerate(PERSONAS):
        ...
        res = simulate(p, i)
        for m in METHODS:
            tot = res["total"]; red = res["signals"][m].get("red", 0)
            print(f"  {m}: red={red} ({red/tot*100:.1f}%) Δ={res['delta'][m]:+d}")
        results.append(res)
